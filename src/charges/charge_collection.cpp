...
ChargeSet::ChargeSet(std::string method_id,
                     std::vector<ChargeAssignment> assignments,
                     std::optional<std::string> parameter_set_id)
    : method_id_{std::move(method_id)},
      parameter_set_id_{std::move(parameter_set_id)},
...
auto ChargeCollection::charge_set(const std::size_t index) const -> const ChargeSet&
{
