...
[[nodiscard]] auto interaction(const double distance, const double width_i, const double width_j)
    -> double {
    if (distance <= 0.0) {
        throw std::logic_error{"SQE+q0 requires positive interatomic distances"};
    }
...
auto SQEq0Method::calculate(const CalculationInput& input) const -> charges::AtomicCharges {
    ...
    Eigen::VectorXd initial_charges = Eigen::VectorXd::Zero(static_cast<Eigen::Index>(atom_count));

    for (std::size_t atom_index = 0; atom_index < atom_count; ++atom_index) {
        initial_charges(static_cast<Eigen::Index>(atom_index)) =
            molecule.atom(atom_index).formal_charge();
    }
