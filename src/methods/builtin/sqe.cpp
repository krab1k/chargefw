...
[[nodiscard]] auto interaction(const double distance, const double width_i, const double width_j)
    -> double {
    if (distance <= 0.0) {
        throw std::logic_error{"SQE requires positive interatomic distances"};
    }

    const auto width_sum = 2.0 * width_i * width_i + 2.0 * width_j * width_j;

    if (width_sum <= 0.0) {
        throw std::logic_error{"SQE atom parameters 'width' must not both be zero"};
    }
...
auto SQEMethod::calculate(const CalculationInput& input) const -> charges::AtomicCharges {
    ...
    if (bond_count == 0) {
        return charges::AtomicCharges{std::vector<double>(atom_count, 0.0)};
    }
    ...
    const Eigen::VectorXd charges = transfer_matrix.transpose() * split_charge;
