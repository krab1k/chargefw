...
auto validate_mapping_size(std::span<const std::size_t> indices,
                           ...
                           const std::string_view context) -> void
{
    ...
    if (indices.size() != expected_size) {
        throw std::invalid_argument{
            std::string{context} + " classification size does not match molecule"
        };
    }
}
                                      ...
                                      const std::string_view context) -> void
{
    for (const auto parameter_entry_index : indices) {
        if (parameter_entry_index >= parameter_entry_count) {
            throw std::invalid_argument{
                std::string{context} + " classification refers to an unknown parameter entry"
            };
        }
    }
}
                                       ...
                                       const ParameterClassification& classification) -> void
{
    ...
    validate_mapping_size(atom_indices, molecule.atom_count(), "atom");
    ...
    validate_mapping_size(bond_indices, molecule.bond_count(), "bond");
