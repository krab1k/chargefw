    ...
    try {
        ...
        if (bonds_value != nullptr) {
            ...
            for (std::size_t index = 0; index < bonds_value->size(); ++index) {
                ...
                if (endpoints.size() != 2) {
                    throw std::runtime_error{bond_context +
                                             ".atoms must contain exactly two indices"};
                }
                bonds.emplace_back(
            ...
            for (std::size_t index = 0; index < conformers_value->size(); ++index) {
                ...
                for (std::size_t coordinate_index = 0; coordinate_index < coordinates.size();
                    ...
                    positions.push_back(core::Position{
                        .x = require_number(coordinate[0], coordinate_context + "[0]"),
...
auto JsonReader::next() -> std::optional<::chargefw::adapters::MoleculeRecordResult> {
