...
auto main() -> int {
    ...
    const std::vector candidate_methods{qeq};
    ...
    assert(charge_set.method_id() == std::string_view{"qeq"});
    assert(charge_set.parameter_set_id().has_value());
    assert(*charge_set.parameter_set_id() == std::string_view{"test-qeq"});

    assert(charges.size() == 3);
    assert_close(charges[0], -0.22710866);
    assert_close(charges[1], 0.11355540);
    assert_close(charges[2], 0.11355326);
