...
auto main() -> int {
    ...
    chargefw::test::assert_calculation_provenance(charge_set, "sfkeem", "test-sfkeem");
    chargefw::test::assert_conformer_dependent(charge_set, 2);

    assert(charges.size() == 3);
    chargefw::test::assert_close(charges[0], -0.13458150, 1.0e-8);
    chargefw::test::assert_close(charges[1], 0.06729732, 1.0e-8);
    chargefw::test::assert_close(charges[2], 0.06728418, 1.0e-8);
