...
auto make_iron_atom() -> core::Molecule {
    std::vector atoms{core::Atom{26, 0, "Fe"}};

    return core::Molecule{std::move(atoms), {}, {}, "iron"};
}
...
auto main() -> int {
    ...
    const auto water_check = veem->check_method_prerequisites(
        {.prepared_molecule = prepared_water, .method_options = options});
    ...
    const auto iron_check = veem->check_method_prerequisites(
        {.prepared_molecule = prepared_iron, .method_options = options});
