...
auto main() -> int {
    ...
    chargefw::test::assert_calculation_provenance(charge_set, "smpqeq", "test-smpqeq");
    chargefw::test::assert_conformer_dependent(charge_set, 2);

    assert(charges.size() == 3);
    chargefw::test::assert_close(charges[0], -0.035475768417, 1.0e-8);
    chargefw::test::assert_close(charges[1], 0.017737997826, 1.0e-8);
    chargefw::test::assert_close(charges[2], 0.017737770591, 1.0e-8);
