...
auto main() -> int {
    ...
    const std::vector candidate_methods{eqeqc};
    ...
    assert(charge_set.method_id() == std::string_view{"eqeqc"});
    assert(charge_set.parameter_set_id().has_value());
    assert(*charge_set.parameter_set_id() == std::string_view{"test-eqeqc"});
